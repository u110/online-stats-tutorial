...
> [!IMPORTANT]
> **t-digestをマージする場合、すべてのノードで `compression` パラメータを統一する必要があります。**

#### なぜcompressionを統一する必要があるのか？

`compression` パラメータはクラスタの粒度を決定します。異なる圧縮率でマージすると：
...
> [!WARNING]
> P²アルゴリズムは**マージできません**。分散システムでパーセンタイル推定が必要な場合は、t-digestまたはGKアルゴリズムを使用してください。

| アルゴリズム | マージ可能 | 分散システム向け |
|-------------|----------|----------------|
| P² | ❌ | 不向き |
| t-digest | ✅ | 推奨 |
| GK | ✅ | 可能 |
